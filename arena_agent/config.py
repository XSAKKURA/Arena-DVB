...
class Settings:
    ...
    # Арена разрешает 8 заочных столов, и мы берём все восемь. Раньше один слот
    # держали в запасе — на случай, если стол, который мы считаем закрытым, на
    # самом деле жив. Запас больше не нужен и не бесплатен: заочная линия и есть
    # почти весь наш оборот матчей, а именно оборота нам не хватает против
    # лидеров таблицы. Расхождение с ареной закрывается само — при старте seats
    # перечитываются из /api/keys/me, ходы ведёт /api/my/turns, а отказ открыть
    # девятый стол приходит как 409 с кодом, за который мы просто садимся.
    max_async_tables: int = 8
