...
class Runner:
        ...
        for table in joinable:
            ...
            if pace == "async":
                if len(self.async_sessions) >= self.settings.max_async_tables:
                    continue
            elif not self.settings.enable_live_lane:
                # `--no-live` до сих пор запрещал только открывать живые столы,
                # а подсаживаться к чужим — нет. Разница видна не в коде, а в
                # таблице: живой стол требует хода каждые пятнадцать минут, и
                # там, где процесс между пробуждениями спит, каждый такой стол
                # рано или поздно становится поражением по времени. Флаг обязан
                # значить одно и то же с обеих сторон.
                continue
    ...
    #: После скольких подряд неудавшихся запросов процесс сдаётся и выходит.
    #: Каждый из них — это уже четыре попытки с отступом внутри клиента, так
    #: что до порога проходит несколько минут: короткий обрыв связи его не
    #: достанет.
    GIVE_UP_AFTER_FAILURES = 12
    ...
    def run_forever(self) -> None:
        ...
        while not self.stopping:
            try:
                ...
                sleep_for = 5.0
            if self.client.transport_failures >= self.GIVE_UP_AFTER_FAILURES:
                # Процесс переживает пересборку машины, а его окружение — нет:
                # адрес прокси меняется, старый перестаёт слушать, и агент
                # бесконечно упирается в «connection refused», оставаясь при
                # этом живым для сторожа. Единственный способ взять новое
                # окружение — родиться заново, поэтому выходим и даём сторожу
                # поднять нас с ним.
                log.error(
                    "%d запросов подряд не дошли до арены — выходим, чтобы сторож "
                    "поднял процесс с новым окружением",
                    self.client.transport_failures,
                )
                return
