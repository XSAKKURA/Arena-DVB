...
_PATTERN_SCORES: list[tuple[str, int]] = [
    ...
    ("11011", 120_000),
    ("11101", 120_000),
    # Открытая тройка стоит почти как закрытая четвёрка, и это не описка.
    # Обе угрозы форсированы одинаково: четвёрку нужно закрыть, иначе будет
    # пять, а тройку — иначе будет открытая четвёрка, которая не закрывается
    # вовсе. Здесь стояло 20 000 против 120 000, и разница в шесть раз стоила
    # рейтинговой партии: соперник спокойно копил тройки, пока одна из них не
    # совпала по времени с четвёркой, — тогда закрывать пришлось четвёрку, а
    # тройка превратилась в открытую четвёрку и решила партию.
    #
    # Ниже четвёрки они остаются намеренно: если выбирать приходится, закрывают
    # сначала четвёрку — она выигрывает на ход раньше.
    #
    # Замер: 120 партий самоигры против прежних весов дали 62,9% при
    # стандартной ошибке 4,6% (интервал ±2σ: 53,8%..72,0%).
    ("01110", 70_000),
    ("010110", 63_000),
    ("011010", 63_000),
    ("001112", 1_200),
